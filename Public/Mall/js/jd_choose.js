...
$(function(){
    ...
    $('.yes_bt').click(function(){
        ...
        if(brand_total>1){
		  ...
		  if(site_rewrite > 0 && brand_url.indexOf("search") < 0){ //伪静态
			  ...
			  burl=brand_url.substring(brand_url.indexOf("brand_id"));
			  console.log(burl);
			  bra="brand="+bra;
    ...
    $('.brand_div .brand_img_word:gt(6)').css('display','none');  // 隐藏大于8索引的品牌图片保留需要的1排
    ...
    choose_open.click(function(){
		...
		function All(){
			if(attr_list.hasClass("extend")){
				...
				//2017模板新增判断
				brands.find('.s-l-value .brand_div .brand_img_word:gt(6)').hide();
