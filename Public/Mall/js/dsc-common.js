...
//	$("#site-nav").jScroll();
	...
	$(".crumbs-nav-item .menu-drop").hover(function(){
		_this_nav = $(this);
		if(_this_nav.find("a span").text()!="首页"){
            _this_nav.addClass("menu-drop-open");
		}else{
            _this_nav.addClass("menu-drop-open-noMenu");
		}
	},function(){
        _this_nav.removeClass("menu-drop-open,menu-drop-open-noMenu");
	});
	...
	//头部搜索tab
	$(".dsc-search-tab li").click(function(){
...
//		}
	...
	//购物车批量删除
	$("*[data-divid='cart-remove-batch']").click(function(){
		var user_id = $("#user_id").val();
		//if(user_id > 0){
			...
			if(divId == 'cart-remove-batch'){
				...
				pbDialog(content,"",0,450,50,"",true,function(){
					Ajax.call('/Mall/Cart/deleteCartGoods', 'goods_id='+cart_value, function(res){
						if(res.error==0){
							location.reload();
						}
					}, 'POST', 'JSON');
...
//		}
	...
	$(document).on("click","*[ectype='cs-w-item']",function(){
		...
		var add_str = "";
		$this.parents(".consignee-warp").find("*[ectype='cs-w-item']").removeClass("cs-selected").find("input[name='consignee_radio']").prop("checked",false);
		$this.addClass("cs-selected").find("input[name='consignee_radio']").prop("checked",true);
		
		add_str += $this.find(".username").text()+"&nbsp;&nbsp;";
		add_str += $this.find(".item-tel .contact").text();
		$("#sendAddr").html("寄送至："+$.trim($this.find(".item-address").text()));
		$("#sendMobile").html("收货人：&nbsp;&nbsp;"+add_str);
	...
	$(document).on("click","*[ectype='dialog_checkout']",function(){
		...
		if(obj.divId == 'new_address'){
            var user_id = $("#site-nav").data("userid");
            if(user_id==0){
                user_login();
                return false;
			}
			if((length+1) >= 11){
				pbDialog("收货地址最多只能添加10条","",0);
				return false;
			}
		}
		if(obj.divId == 'new_address' || obj.divId == 'edit_address'){
			if(obj.divId == 'new_address'){
				//添加收货地址信息
				Ajax.call(obj.url, '', function(data){
					pb({
						id:obj.divId,
						title:obj.title,
						width:obj.width,
				...
				//修改收货人地址
				var address_id = $(this).attr("data-itemsAddressId");
				Ajax.call(obj.url, 'address_id='+address_id, function(data){
					pb({
						id:obj.divId,
						title:obj.title,
						width:obj.width,
					...
					if(province!=""){
						...
						if(city!=""){
							setTimeout(function(){
								getRegion(province,1,city);	
								if(district!=""){
									setTimeout(function(){
										getRegion(city,2,district);
									},60);						
								}
							},50);					
						}
			...
			pb({
				...
				onOk:function(){
					Ajax.call(obj.url, 'address_id=' + address_id, function(data){						
						if(data.error==0){
							Ajax.call('/Mall/Cart/addressList', '', function(res){
								$("#consignee-addr").html(res);
							}, 'POST', 'html');
						}
					}, 'POST','JSON');
				}
			});
		}else if(obj.divId=="set_default"){
			var address_id = $(this).attr("data-itemsAddressId");
			//设为默认收货地址
			var content = $('#set_default').html();
			...
			if(data.error == 0){
				pb({
					...
					onOk:function(){
						...
						Ajax.call('ajax_dialog.php?act=gotoInvoice','inv_content='+encodeURIComponent(inv_content)+'&invoice_id='+invoice_val +'&warehouse_id='+warehouse_id +'&area_id='+area_id +'&store_id='+store_id +'&invoice_type='+invoice_type + '&tax_id=' + tax_id + '&shipping_id=' + $.toJSON(shipping_id),gotoInvoiceResponse, 'POST', 'JSON');
...
/* 未登录弹出框 start */
jQuery.notLogin = function(actUrl){
	Ajax.call(actUrl,'', function(data){
		pb({
			id:"loginDialogBody",
...
function addUpdate_Consignee(frm){
	...
	if(csg.goods_flow_type == 101){
		...
		csg.id_number 		= obj.find("[name='id_number']").val();
	}
	...
	if(csg.consignee == ''){
		...
		Ajax.call('/Mall/Cart/addUserAddress', 'csg=' + JSON.stringify(csg) , addUpdate_ConsigneeResponse, 'POST', 'JSON');
		fale = true;
	}
...
function addUpdate_ConsigneeResponse(result){
	if(result.error==0){
		Ajax.call('/Mall/Cart/addressList', '', function(res){
			$("#consignee-addr").html(res);
//			$("#consignee-addr .address-list").last().find(".cs-w-item ").addClass("cs-selected").find("input[name='consignee_radio']").prop("checked",true);
		}, 'POST', 'html');
	}
...
function checked_store_info(){
	...
	var cart_value = $("input[name='goods_id']").val();
...
function getRegion(_id,_type,_val){	
	Ajax.call("/Mall/Cart/region","id="+_id,function(res){
		if(_type==0){
			var str = '';
			for(var i=0;i<res.length;i++){
				str+='<div onclick="getRegion('+res[i].region_id+',1)" class="option" data-value="'+res[i].region_id+'" data-text="'+res[i].local_name+'" data-type="'+res[i].region_id+'" ectype="ragionItem">'+res[i].local_name+'</div>'	;
				if(_val&&_val==res[i].region_id){
					$("#selProvinces_ .txt").html(res[i].local_name);
				}
			}
			$("#selProvinces_ dd").html(str);
			
		}
		if(_type==1){
			$("#selDistricts_ dd").html("");
			var str = '';
			for(var i=0;i<res.length;i++){
				str+='<div onclick="getRegion('+res[i].region_id+',2)" class="option" data-value="'+res[i].region_id+'" data-text="'+res[i].local_name+'" data-type="'+res[i].region_id+'" ectype="ragionItem">'+res[i].local_name+'</div>'	;
				if(_val&&_val==res[i].region_id){
					$("#selCities_ .txt").html(res[i].local_name);
				}
			}
			$("#selCities_ dd").html(str);			
		}
		if(_type==2){
			var str = '';
			for(var i=0;i<res.length;i++){
				str+='<div class="option" data-value="'+res[i].region_id+'" data-text="'+res[i].local_name+'" data-type="'+res[i].region_id+'" ectype="ragionItem">'+res[i].local_name+'</div>';
				if(_val&&_val==res[i].region_id){
					$("#selDistricts_ .txt").html(res[i].local_name);
				}
			}
			$("#selDistricts_ dd").html(str);			
		}
