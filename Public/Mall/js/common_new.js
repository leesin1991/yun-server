...
//}
    ...
    if(user_id>0){
        Ajax.call('/Mall/cart/addcart/', 'goods_id=' + goodsId+'&goods_number='+number+'&divId='+divId, addToCartShowDivResponse, 'POST', 'JSON');
    }else{
        $.notLogin('/mall/Public/notLogin');
    }
...
function addToCartShowDivResponse(result)
{
	if(result.error==0){
		Ajax.call('/Mall/Cart/cartInfo','', function(res){			
			var str = '<div class="center_pop_txt"><div class="title">'
		    +'<h3>提示</h3>'
		...
		+'<div class="center_pop_p">'
		    ...
		    +'<div class="desc">'
		        +'<span>购物车共有</span>'
		        +'<strong>'+res.num+'</strong>'
		        +'<span>宝贝总金额为：</span>'
		        +'<em class="saleP">￥'+res.totle+'</em>'
		    +'</div>'
		+'</div>'
		+'</div>';
		$("#addtocartdialog .center_pop_txt").html(str);		
			...
			$("#ECS_CARTINFO").data("carteveval", 0);
		}, 'POST', 'JSON');		
		
	}else{
        get_goods_prompt_message(result.message);
    }
//if (result.error > 0)
//{
//  // 如果需要缺货登记，跳转
...
//	});
	...
	//if($(".goods_info_attr").length > 0){
		//if(attr_list == is_selected){
			//Ajax.call('/Mall/cart/addcart/', 'goods=' + $.toJSON(goods), addToCartResponse, 'POST', 'JSON');
		//}else{
			//get_goods_prompt_message(json_languages.Product_spec_prompt);
		//}
//	}else{
    var user_id = $("#site-nav").data("userid");
        if(user_id>0){
            Ajax.call('/Mall/cart/addcart/', 'goods_id=' + goodsId+'&goods_number='+number+'&divId='+divId, addToCartResponse, 'POST', 'JSON');
        }else{
            $.notLogin('/mall/Public/notLogin');
        }
