...
function get_url($path){
    ...
    return 'http://'.$bucket.$end_point.$path;
}
/**
 * 删除oss上指定文件
...
function oss_delet_object($object){
    ...
    // 获取bucket名称
    $bucket=C('ALIOSS_CONFIG.BUCKET');
    $test=$oss->deleteObjects($bucket,$object);
}
...
//地址联动
function getRegion($parent_id = 0,$flag = false)
{
    $condition['p_region_id'] = $parent_id;
    $region = M('p_region')->where($condition)->select();
    if ($flag) {
