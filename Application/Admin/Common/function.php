...
function get_goods_brand(){
    ...
    $map['status'] = 1;
    $goods_barnd = M('b_brand_info')->join('left join b_store_info on b_store_info.store_id = b_brand_info.store_id')->where($map)->select();
    return $goods_barnd;
}
...
function getCategoryName($category_id = 0){
    $conditon['id']= $category_id;
    $res = M("m_category_info")->where($conditon)->getField('name');
    return $res;
}

function getDetailCategoryName($category_id = 0){
    $conditon['parent_id']= $category_id;
    $res = M("m_category_info")->where($conditon)->select();
    foreach ($res as $k=>$v){
        $arr[] = $v['cat_name'];
    }
    $str = implode('、',$arr);
...
function get_regin_name($region_id = 0){
    $condition['region_id'] = $region_id;
    $res = M("b_region")->where($condition)->getField('local_name');
    return $res;
}
/**
 * 系统非常规MD5加密方法
 ...
 * @return string
 */
function think_ucenters_md5($str, $key = 'ThinkUCenter'){
