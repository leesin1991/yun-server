...
class AuthManagerController extends AdminController{
    ...
    public function index(){
        $list = $this->lists('MAuthGroup',array(),'id asc');
    ...
    public function editGroup(){
        $auth_group = M('MAuthGroup')->find( (int)$_GET['id'] );
    ...
    public function access(){
        $this->updateRules();
        $auth_group = M('MAuthGroup')->where( array('status'=>array('egt','0')) )
                                    ->getfield('id,id,title,rules');
        $node_list   = $this->returnNodes();
        $map         = array('type'=>AuthRuleModel::RULE_MAIN,'status'=>1);
        $main_rules  = M('MAuthRule')->where($map)->getField('name,id');
        $map         = array('type'=>AuthRuleModel::RULE_URL,'status'=>1);
        $child_rules = M('MAuthRule')->where($map)->getField('name,id');
    ...
    public function changeStatus($method=null){
        ...
        switch ( strtolower($method) ){
            case 'forbidgroup':
                $this->forbid('MAuthGroup');
                break;
            case 'resumegroup':
                $this->resume('MAuthGroup');
                break;
            case 'deletegroup':
                $this->delete('MAuthGroup');
                break;
            default:
