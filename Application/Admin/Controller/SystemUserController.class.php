...
//        echo '<pre>';print_r($list);exit;
        ...
        $sql = M('m_member_info')->alias('i')
            ->field('m.*,g.title,i.status,i.user_name')
            ->join('left join m_auth_group as g on g.id = i.group_id')
            ->join('left join m_member_login as m on i.uid = m.id')
            ->where($map)
            ->order('id DESC')
            ->buildSql();
//        print_r($sql);exit;
...
//        echo '<pre>';print_r($business_info);exit;
    ...
    public function add(){
        ...
        if(IS_POST){
            ...
            $user = array('uid' => $arr, 'nickname' => $username,'user_name'=>$name, 'status' => 1,'group_id'=>$per,'reg_time'=>time());
            if(M('m_member_info')->add($user) > 0 && $flag){
                M()->commit();
                echo 1;exit;
            }else{
                M()->rollback();
    ...
    public function edit(){
        if(IS_POST){
            ...
            $res = M('m_member_login')->where('id='.$user_id)->save($ucenter_member);
            $member = array('nickname' => $user_name,'user_name'=>$nickname,'group_id'=>$role);
            $resu = M('m_member_info')->where('uid='.$user_id)->save($member);
            if($res || $resu){
                echo 1;exit;
            }else{
                echo -1;exit;
    ...
    public function is_edit($id){
        $user_list = M('m_member_info')->alias('i')
            ->field('m.*,g.id as gid,g.title,i.status,i.user_name')
            ->join('left join m_auth_group as g on g.id = i.group_id')
            ->join('left join m_member_login as m on i.uid = m.id')->where('m.id='.$id)->find();
        if($user_list){
    ...
    public function changestatus(){
        ...
        M("m_member_info")->where($where)->save($data);
        $this->success('操作成功！');
    ...
    public function restart($id){
        ...
        $data = array('password' => $password,'update_time'=>time());
        $res = M('m_member_login')->where('id='.$id)->save($data);
        if($res){
            echo 1;exit;
        }else{
            echo 2;exit;
    ...
    public function get_role(){
        $uid = UID;
        if($uid == 1){
            $role_list = M('m_auth_group')->field('id,title')->select();
        }else{
            $map['u.id'] = $uid;
            $role_list = M('m_member_info')->alias('m')->join('left join m_auth_group as g on g.id = m.group_id')->field('g.id,g.title')->where($map)->find();
        }
