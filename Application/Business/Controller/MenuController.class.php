...
class MenuController extends AdminController {
    ...
    public function index(){
        $pid  = I('get.pid',0);
        if($pid){
            $data = M('b_menu')->where("id={$pid}")->field(true)->find();
            $this->assign('data',$data);
        }
        $title      =   trim(I('get.title'));
        $type       =   C('CONFIG_GROUP_LIST');
        $all_menu   =   M('b_menu')->getField('id,title');
        $map['pid'] =   $pid;
        if($title)
            $map['title'] = array('like',"%{$title}%");
        $list       =   M('b_menu')->where($map)->field(true)->order('sort asc,id asc')->select();
    ...
    public function add(){
        if(IS_POST){
            ...
            $menus = M('b_menu')->field(true)->select();
    ...
    public function edit($id = 0){
        if(IS_POST){
            ...
            $info = array();
            /* 获取数据 */
            $info = M('b_menu')->field(true)->find($id);
            $menus = M('b_menu')->field(true)->select();
            $menus = D('Common/Tree')->toFormatTree($menus);
    ...
    public function sort(){
        if(IS_GET){
            ...
            $list = M('b_menu')->where($map)->field('id,title')->order('sort asc,id asc')->select();
            ...
            foreach ($ids as $key=>$value){
                $res = M('b_menu')->where(array('id'=>$value))->setField('sort', $key+1);
            }
            if($res !== false){
                $this->success('排序成功！');
            }else{
