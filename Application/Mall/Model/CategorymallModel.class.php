...
class CategoryMallModel extends Model{
    ...
    protected $trueTableName = 'm_category_info';
    ...
    public function update(){
        ...
        //记录行为
        action_log('update_category', 'categorymall', $data['id'] ? $data['id'] : $res, UID);
     ...
     * @author 麦当苗儿 <zuojiazi@vip.qq.com>
     * */
    public function getLastCategory($category_id = 0)
    {
        $condition['id'] = $category_id;
        $res = M("m_category_info")->where($condition)->getField('id,pid,name');
        $name = M("m_category_info")->where(array('id' => $res[$category_id]['pid']))->getField('name');
        $arr =  array('top' => $name ,'crrent' => $res[$category_id]['name']);
        return $arr;
    }
    /**
     * 根据类目ID获取所有子类ID，包含自身ID
     */
    public function getChild($category_id = 0){
        if(!$category_id) return false;
        $map['pid'] = array('eq',$category_id);
        $arr = $this->where($map)->getField('id',true);
        if(!$arr){
