...
class RedisController extends HomeController {
     ...
     * @return boolean 
     */  
    public function connect($config=array('host'=>'192.168.3.129','port'=>6379,'password' => 'yihengyun'), $isMaster=true){  
        // default port  
    ...
    public function isOutlimit($key, $limit=60){
        $check = $this->getRedis()->exists($key);  
        if($check){  
            $this->getRedis()->incr($key);  
            $count = $this->getRedis()->get($key);  
            if($count > $limit){  
                exit(jsonError('10006','Your have too many request'));  
            }  
        }else{  
            $this->getRedis()->incr($key);  
            //限制时间为60秒   
