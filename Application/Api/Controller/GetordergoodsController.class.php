...
class GetordergoodsController extends HomeController
{
    public function getorder(){
        ...
        for ($i = 0;$i<$nums;$i++) {
            ...
            foreach ($arr['data'] as $k => $v) {
                $con['account_id'] = 42;
                $con['order_sn'] = $v['order_sn'];
                $con['order_sn_child'] = $v['order_sn'];
                $con['add_time'] = time();
                $order_parent = M('b_order_parent')->where(array('order_sn'=>$v['order_sn']))->find();
                if ($order_parent) {
                    $con['add_time'] = time();
                    $resu = M('b_order_parent')->where(array('order_sn'=>$v['order_sn']))->save($con);
                } else {
                    $resu = M('b_order_parent')->add($con);
                }
                ...
                if ($order_list) {
                    ...
                    $res = M('b_order_info')->where(array('order_sn' => $v['order_sn']))->save($data);
    ...
    private function getorder_count(){
        $page_url = "http://test.api.com/v1/getordergoods/getpages?";
//        $page_url = "http://106.14.93.24/v1/getordergoods/getpages?";
        $res = http($page_url);
        return $res;
    }

    public function get_order_info($page='',$pagesize=''){
        $url = "http://test.api.com/v1/getordergoods/index";
//        $url = "http://106.14.93.24/v1/getordergoods/index";
        $param['page'] = $page;
    ...
    public function getorders(){
        $order_arr = M('b_order_info')->where(array('account_id'=>42))->order('order_id desc')->limit(1)->find();
...
//        $page_url = "http://106.14.93.24/v1/getorderinfo/getorderpages";
    ...
    public function get_orders_info($order_sn='',$page='',$pagesize=''){
        $url = "http://test.api.com/v1/getorderinfo/ordergoods";
//        $url = "http://106.14.93.24/v1/getorderinfo/ordergoods";
        $param['order_sn'] = $order_sn;
        $param['page'] = $page;
        $param['pagesize'] = $pagesize;
        $response = http($url,$param);
